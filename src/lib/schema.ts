...
/**
 * One freely-licensed picture per group, generated by scripts/fetch-images.mjs.
 * Kept out of nodes.json deliberately: that file is hand-authored prose, this one
...
export const ImageSchema = z.object({
  ...
  // not decoration, so the schema makes them non-optional.
  author: z.string().min(1),
  license: z.string().min(1),
  licenseUrl: z.string().url(),
  sourceUrl: z.string().url(),
  // Required so the <img> can reserve its box before the bytes arrive; without
  // them a lazily-loaded image shifts everything below it as it lands.
  width: z.number().int().positive(),
  height: z.number().int().positive(),
});
export type EvoImage = z.infer<typeof ImageSchema>;
export const ImagesSchema = z.record(z.string(), ImageSchema);
export type EvoImages = z.infer<typeof ImagesSchema>;
